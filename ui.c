...
extern LPSTR lan_menucmptotext;
extern LPSTR lan_menucmptohtml;


char USERSSTRING_LONG[]="HKEY_CURRENT_USERS";  //1.6 using long name, so in 1.8.1 an option in regshot.ini "UseLongRegHead" control this
char LOCALMACHINESTRING_LONG[]="HKEY_LOCAL_MACHINE";
char USERSSTRING[]="HKCU"; //="HKEY_CURRENT_USERS"; // 1.7 using short name tfx, a good idea
...
{
	hMenu=CreatePopupMenu();
	AppendMenu(hMenu,MF_STRING,IDM_CMPTOTEXT,lan_menucmptotext);
	AppendMenu(hMenu,MF_STRING,IDM_CMPTOHTML,lan_menucmptohtml);
	SetMenuDefaultItem(hMenu,IDM_CMPTOTEXT,FALSE);
}
