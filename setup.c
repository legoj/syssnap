...
LPSTR	INI_INCVERSIONINFO	="IncludeVerInfo";
...
BOOL GetSnapRegs(HWND hDlg) 
{
	...
	//if(nFlag!=0)
	{
		...
		//SendMessage(GetDlgItem(hDlg,IDC_CHECKDIR),BM_SETCHECK,(WPARAM)((nFlag&0x02)>>0),(LPARAM)1);
		SendMessage(GetDlgItem(hDlg,IDC_CHECKDIR),BM_SETCHECK,(WPARAM)BST_CHECKED,(LPARAM)1);
...
BOOL SetSnapRegs(HWND hDlg) //tfx 保存信息到配置文件
{
	...
	nFlag=(BYTE)(SendMessage(GetDlgItem(hDlg,IDC_CHECKDIR),BM_GETCHECK,(WPARAM)0,(LPARAM)0)<<1);
