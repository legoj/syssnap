/*
    ...
    Foundation, IncC., 51 Franklin St, Fifth Floor, Boston, MA  02110-1301  USA
*/
...
WORD wValueLength; 
WORD wType; 
WCHAR szKey[15];        //-- WCHAR / UNICODE string
WORD Padding1[1];       //-- this is actually dynamic
}VS_VERSIONINFO;

struct _SVCCONTENT{
	LPSTR	lpSvcName;						//service name
	LPSTR	lpDispName;						//display name
	LPSTR	lpPathName;						//path 
	LPSTR	lpStartName;					//svc start name
	DWORD	dwSvcType;						//svc type
	DWORD	dwCurrState;					//svc state
	DWORD	dwStartType;					//svc start type
	DWORD	dwTagId;						//svc tag id
	struct	_SVCCONTENT FAR * lpnextsvc;	//next service content
	BYTE	bsvcmatch;						//flag used for comparing svc attr
};
typedef struct _SVCCONTENT SVCCONTENT,FAR * LPSVCCONTENT;
...
LPSVCCONTENT	lpSvcContent1, lpSvcContent2;			//Pointer to svcs
...
LPSTR	lpWindowsDirName,lpTempPath,lpStartDir,lpIni,lpFreeStrings,lpCurrentTranslator, lpSysDrive;
...
VOID CreateComparePopupMenu(VOID);
VOID	UI_BeforeShot(DWORD id);
VOID	UI_AfterShot(VOID);
VOID	UI_BeforeClear(VOID);
VOID	UI_AfterClear(VOID);
VOID SnapAction(HWND, BOOL, unsigned short);
VOID CompareAction(HWND , BOOL );

VOID Shot1(void);
VOID Shot2(void);
BOOL CompareShots(BOOL);
...
LPSTR	GetFileDetails(LPFILECONTENT lpFileContent);
int GetFileVersion( char * filename, int *maj, int *min, int *rev, int *bld );
