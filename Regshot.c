...
char str_DefResPre[]="SSCompareResult";
char str_filter[]={"Snapshot files [*.snp]\0*.snp\0All files\0*.*\0\0"};
char str_RegFileSignature[]="SYSSNAPFILE"; //Need [] to use with sizeof()
...
VOID	LogToMem(DWORD actiontype,LPDWORD lpcount,LPVOID lp)
{
	...
	else
	{
		if(actiontype==VALADD||actiontype==VALDEL||actiontype==VALMODI)
		{
			...
			lpall=MYALLOC(strlen(lpname)+strlen(lpdata)+4);  
...
//------------------------------------------------------------
BOOL CompareShots(BOOL isHTML)
{
	BOOL	bshot2isnewer;//,bSaveWithCommentName;
